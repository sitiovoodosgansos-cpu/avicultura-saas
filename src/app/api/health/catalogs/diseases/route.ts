...
import { createDisease, listDiseases } from "@/lib/health/catalogs";
...
export async function GET() {
  ...
  const data = await listDiseases(auth.session.user.tenantId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const created = await createDisease(auth.session.user.tenantId, parsed.data);
