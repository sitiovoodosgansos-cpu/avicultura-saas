...
export async function POST(request: NextRequest) {
  ...
  if (body && typeof body === "object" && "flockGroupIds" in body) {
    const parsed = multiFlockVaccinationSchema.safeParse(body);
    if (!parsed.success) {
    ...
    try {
      const created = await recordMultiFlockVaccination(auth.session.user.tenantId, parsed.data);
